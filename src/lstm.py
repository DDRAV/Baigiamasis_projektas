...
def build_lstm_model(input_shape, units=128, dropout_rate=0.2, learning_rate=0.001):
    model = Sequential([
        ...
        Reshape((input_shape[0], 1)),  # Add time dimension
        ...
        LSTM(units),
        Dropout(dropout_rate),
        Dense(1, activation="linear")  # Predicting one chord at a time
    ])
...
param_grid = [
    {"units": 128, "dropout_rate": 0.4, "learning_rate": 0.001, "epochs": 100, "batch_size": 128}  # Increased epochs for early stopping
]
